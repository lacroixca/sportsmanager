		...
		if (in_array('all', $stat_v[2]) || in_array($sport, $stat_v[2])) {
			...
			foreach ($rows as $row) {
				...
				if (isset($row->$stat_k) && $row->$stat_k > 0) {
					if (in_array($stat_k, array ('batting_average', 'on_base_percentage')) && $row->at_bat < $min_games_played) continue;
					$stat_leaders[] = $row;
					$i++;
				} else {
					if ($i >= 1) break;
			...
			$leader = isset($stat_leaders[0]) ? $stat_leaders[0] : '';
...
<?php
	$i = 0;
	foreach ($stat_leaders as $stat_leader) {
		$i++;
		if (count($stat_leaders) > 0) {
?>

<tr>
	<td><?php echo $i; ?></td>
	<td><?php echo isset($stat_leader->player_link) ? $stat_leader->player_link : ''; ?></td>
	<td><?php echo $stat_leader->$stat_k; ?></td>
</tr>
